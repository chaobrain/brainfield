...
class AlnModel(brainstate.nn.Dynamics):
    ...
    def calulate_mean_synaptic_gating_variance(self, z1ee,z1ei,z1ie,z1ii,z2ee, z2ei, z2ie, z2ii):
        d_seev = ((1 - self.seem.value)**2 * z2ee + (z2ee - 2 * self.tau_se * (z1ee + 1)) * self.seev.value) / self.tau_se**2
        d_seiv = ((1 - self.seim.value)**2 * z2ei + (z2ei - 2 * self.tau_si * (z1ei + 1)) * self.seiv.value) / self.tau_si**2
        d_siev = ((1 - self.siem.value)**2 * z2ie + (z2ie - 2 * self.tau_se * (z1ie + 1)) * self.siev.value) / self.tau_se**2
        d_siiv = ((1 - self.siim.value)**2 * z2ii + (z2ii - 2 * self.tau_si * (z1ii + 1)) * self.siiv.value) / self.tau_si**2
    ...
    # Interpolate from lookup tables
        z1ee,z1ei,z1ie,z1ii,z2ee,z2ei,z2ie,z2ii,r_exc_kHz, r_inh_kHz= self.calculate_total_input_firing_rate(rowsum, rowsumq)
        ...
        d_seev, d_seiv, d_siev, d_siiv = self.calulate_mean_synaptic_gating_variance(z1ee,z1ei,z1ie,z1ii,z2ee, z2ei, z2ie, z2ii)

        self.seem.value += brainstate.nn.exp_euler_step(lambda seem: ((1 - seem) * z1ee - seem) / self.tau_se, self.seem.value)
        self.seim.value += brainstate.nn.exp_euler_step(lambda seim: ((1 - seim) * z1ei - seim) / self.tau_si, self.seim.value)
        self.siem.value += brainstate.nn.exp_euler_step(lambda siem: ((1 - siem) * z1ie - siem) / self.tau_se, self.siem.value)
        self.siim.value += brainstate.nn.exp_euler_step(lambda siim: ((1 - siim) * z1ii - siim) / self.tau_si, self.siim.value)
