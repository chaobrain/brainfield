            ...
            for i_batch in range(num_batches):
                ...
                if self.model.model_name == 'WWD':
                    ...
                    loss_EI = 0.1 * torch.mean(
                        torch.mean(
                            E_batch * torch.log(E_batch) +
                            (con_1 - E_batch) * torch.log(con_1 - E_batch) +
                            0.5 * I_batch * torch.log(I_batch) +
                            0.5 * (con_1 - I_batch) * torch.log(con_1 - I_batch),
                            axis=1
                        )
                    )
                else:
                ...
                if self.model.model_name == 'WWD':
                    loss = (
                        0.1 * w_cost * self.cost.cost_eff(self.model, next_batch['bold_batch'], ts_batch)
                        + sum(loss_prior)
                        + 0.5 * loss_EI
                    )
                elif self.model.model_name == 'JR':
                    loss = w_cost * self.cost.cost_eff(self.model, next_batch['eeg_batch'], ts_batch)
...
plt.plot(ki0)
# %% md
# Please download the individual leadfield matrix at:
#
# https://drive.google.com/drive/folders/11jOvrzupbm2W8QdKTa9gPeG2Ivk4BdMN?usp=sharing
#
# and replate the path in the cell below:
#
# file_leadfield = path/to/your/leadfield_from_mne
# %%
...
for i in range(1):
    ...
    file_leadfield = f'{files_dir}/leadfield_from_mne/sub' + str(i + 1).zfill(3) + '/leadfield.npy'
    lm = np.load(file_leadfield, allow_pickle=True)
    ...
    par = ParamsJR(
        ...
        v0=[6, 0],
        r=[0.56, 0],
        y0=[2 * np.ones((output_size, 1)), 2 * np.ones((output_size, 1))],
        mu=[1., 2.5],
        k=[10, .3],
        cy0=[5, 0],
        ki=[ki0, 0],
    ...
    filename = 'reproduce_fig/sub_' + str(i) + '_fittingresults_stim_exp.pkl'
    with open(filename, 'wb') as f:
        pickle.dump(F, f)

    outfilename = 'reproduce_fig/sub_' + str(i) + '_simEEG_stim_exp.pkl'
